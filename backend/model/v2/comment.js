...
export const createComment = async (SQLClient, { content, postID, clientID }) => {
    const { rows } = await SQLClient.query(
        "INSERT INTO Comment(content, post_id, client_id) VALUES ($1, $2, $3) RETURNING *",
...
export const getCommentsByPostID = async (SQLClient, {postID}) => {
    const {rows} = await SQLClient.query("SELECT * FROM Comment WHERE post_id = $1", [postID])
    return rows;
}
...
export const getComments = async (SQLClient, { commentDate, page = 1, limit = 10 }) => {
    ...
    try {
        ...
        const dataQuery = `SELECT 
            c.id, c.content, c.date, c.post_id, c.client_id,
